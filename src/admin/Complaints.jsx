...
export default function Complaints() {
  ...
  const handleGetComplaintsData = async () => {
    const accessToken = sessionStorage.getItem("adminToken");
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/admin/customers/viewcomplaints`,
        {
          token: accessToken,
        },
        {
      ...
      // Add customerName to each complaint object in the response data
      const complaintsWithData = response.data.complaints.map((complaint) => {
        return {
          ...complaint,
          customerName: complaint.customerName, // Assuming customer name is nested under 'customer' object
        };
      });
      // Set the tableRows state with the complaints data
      setTableRows(complaintsWithData);
      console.log(complaintsWithData);
    } catch (error) {
  ...
  useEffect(() => {
    // Call handleGetCustomerData when the component mounts
